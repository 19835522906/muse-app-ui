...
- **B · 沉浸内容流（Tezza 式，默认）**：打开先是一段 15s 品牌短片满屏静音循环（定调性的那支：海滩、天台、市集、照片墙），叠一句 "Make it look the way it felt." 和一个 Explore looks；点它或上滑进内容流。内容流一屏一个氛围，照片满屏出血，竖向吸附滑动；进入一屏时 Before → After 扫一次然后停在 After；文字只有 kicker 胶囊（点进主题合集）、模版名、一句效果；每屏同样两个入口 Upload a photo / Shoot；顶栏固定，底部三 Tab 换深色玻璃。参考 Tezza 的 Studio 页：整屏内容 + 一个衬线标题 + 底部一排小动作 + 一个主按钮，全屏就是调性本身。
