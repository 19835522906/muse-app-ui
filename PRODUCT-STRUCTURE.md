...
- **B · 沉浸内容流（Tezza 式，默认）**：打开先是 15s 品牌短片满屏循环 + 一个 Explore 按钮；Explore 或上滑进入内容流，一屏一个氛围，照片满屏出血，没有卡片和货架。见本节末尾
...
- 第 0 屏是品牌短片：15s 竖版（海滩 / 天台 / 市集 / 照片墙），满屏静音循环，右下小钮开声音；叠一句品牌文案 "Make it look the way it felt." 和一个按钮 **Explore looks**。点它或上滑就进内容流；短片只在这一屏在视口内时播放
- 之后竖向整屏吸附滑动，一屏就是一个氛围：照片满屏，进入这一屏时 Before → After 扫一次（停 0.4s，扫 1.15s，只有一条细白线），之后停在 After；滑走再滑回会重放
...
- 第一个 Look 屏在入口上方有一行小字 Swipe up for the next look（短片屏不显示），滑过它之后就消失
...
- 模式二 · Cameras：一排老相机，轮播里每台是一张机身贴纸（不是照片缩略图），选哪台取景器就整套变成那台机器。每台带三样东西：
